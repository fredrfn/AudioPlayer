...
OpenMP3::Decoder::Decoder(const Library & library) : library(library) {
...
OpenMP3::UInt OpenMP3::Decoder::ProcessFrame(const Frame & frame, Float32 output[2][1152]) {
	...
	if (Private::ReadMain(*this, frame)) {
...
bool OpenMP3::Decoder::Private::ReadSideInfo(Decoder & self, const Frame & frame) {
	...
	struct Local {
		FORCEINLINE static UInt32 GetSideBits(UInt n, const UInt8 * & ptr, UInt & idx) {
